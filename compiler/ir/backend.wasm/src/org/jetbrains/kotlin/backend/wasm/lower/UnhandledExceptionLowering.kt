...
internal class UnhandledExceptionLowering(val context: WasmBackendContext) : FileLoweringPass {
    ...
    private fun createSyntheticConstantVariable(name: String, irType: IrType, origin: IrDeclarationOrigin): IrVariable =
        IrVariableImpl(
    ...
    private fun processExportFunction(irFunction: IrFunction) {
        ...
        with(context.createIrBuilder(irFunction.symbol)) {
            val currentIsNotFirstWasmExportCall = createSyntheticConstantVariable(
                name = "currentIsNotFirstWasmExportCall",
                irType = irBooleanType,
                origin = IrDeclarationOrigin.IR_TEMPORARY_VARIABLE
            )

            val e = createSyntheticConstantVariable(
                name = "e",
                irType = throwableType,
    ...
    override fun lower(irFile: IrFile) {
        for (declaration in irFile.declarations) {
            if (declaration is IrFunction && declaration.name.asString() == "box"/* && declaration.isJsExport()*/) {
